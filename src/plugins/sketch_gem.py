...
class SketchGEMPlugin(SupervisedPlugin):
    """
    ...
    def __init__(self, patterns_per_exp: int, memory_strength: float, proj_metric,  sketch_dim: int = 500, sgd_iterations: int = 3):
        """
        ...
        # Random Sketch (dim-reduce param space)
        self.r = sketch_dim
        self.R= None # Tensor
        self.Z = torch.empty(0)
        self.ZZT = torch.empty(0)
        self.sgd_iter = sgd_iterations
    ...
    def before_training_iteration(self, strategy, **kwargs):
        ...
        if strategy.clock.train_exp_counter > 0:
            ...
            if self.R is None:
                T, d = self.G.shape
                self.R = torch.randn(self.r, d, device=strategy.device)
                self.R /= torch.sqrt(torch.tensor(self.r, dtype=self.R.dtype, device=self.R.device))
                
            self.Z = torch.matmul(self.G, self.R.T)
            self.ZZT = torch.matmul(self.Z, self.Z.T)
            eigvals = torch.linalg.eigvalsh(self.ZZT)
            L = eigvals.max()
            self.lr = 1.0 / (L + 1e-6)
    ...
    def after_backward(self, strategy, **kwargs):
        ...
        if strategy.clock.train_exp_counter > 0:
            ...
            if self.R is not None:
                z = torch.mv(self.R, g)   # shape: (r,)
                # (2) test if any sketch‐inner‐product is negative
                to_project = (torch.mv(self.Z, z) < 0).any()
                #to_project = True
    ...
    def _solve_sketch_dualsgd(self, z: Tensor, iters: int = 10) -> Tensor: 
        T, r = self.Z.shape
        v = torch.zeros(T, device=z.device)     # your α
        Gz = torch.mv(self.Z, z)                # shape (T,)
        for _ in range(iters):
            gradF = torch.mv(self.ZZT, v) + Gz
            v = v - self.lr * gradF
