...
class SGEMWarmNITPlugin(SupervisedPlugin):
    """
    ...
    def __init__(self, patterns_per_exp: int, memory_strength: float, sgd_iterations: int, proj_metric, projection_iteration):
        """
        ...
        self.projection_iteration_multiple = projection_iteration
    ...
    def before_training_iteration(self, strategy, **kwargs):
        ...
        if strategy.clock.train_exp_counter > 0:
            ...
            eigvals = torch.linalg.eigvalsh(self.GGT)
            L = eigvals.max()
            self.lr = 1.0 / (L + 1e-6) 
    ...
    def after_backward(self, strategy, **kwargs):
        ...
        if to_project:
            ...
            v_star = self.solve_dualsgd(strategy.clock.train_exp_counter, strategy.device, g, self.sgd_iterations)
    ...
    def solve_dualsgd(self, t, dev, g, I):
        ...
        for _ in range(I):
            ...
            temp = torch.mv(self.GGT, self.v)
            gradF = temp + Gg
            self.v -= self.lr * gradF
            self.v = torch.max(self.v, torch.full_like(self.v, self.memory_strength))
